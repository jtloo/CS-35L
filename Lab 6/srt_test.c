...
void renderImage(void *threadNum)
{
    int columnNumber = *(int*) threadNum;
    ...
    /* Divides the Picture into nthread Columns 
    
     */
    for( int px=columnNumber; px<width; px+=nthreads )
    {
...
main( int argc, char **argv )
{
    ...
    /* The Multithreading Code is Added Here*/
    pthread_t threads[nthreads]; //Creating Array of threads
    
    int *threadNum = malloc(sizeof(int) * nthreads);
    int i, ret;
    for (i = 0; i < nthreads; i++)
    {
        threadNum[i] = i; //This will keep track of which thread number the function is running in
    ...
    for (i=0; i < nthreads; i++)
    {
        ...
        if (ret != 0)
        {
            fprintf(stderr, "Error: Cannot rejoin threds");
            return 1;
        }
    }
